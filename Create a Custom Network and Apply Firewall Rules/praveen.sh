...
section_header() {
    ...
    echo "${CYAN_TEXT}${BOLD_TEXT}  Welcome To PRAVEEN TECH Cloud Tutorials Do like the video $1${RESET_FORMAT}"
    echo "${CYAN_TEXT}${BOLD_TEXT}╚════════════════════════════════════════════════════════╝${RESET_FORMAT}"
    echo
}
...
echo "${GREEN_TEXT}${BOLD_TEXT}All network configurations have been successfully applied by Praveen Tech Guide.${RESET_FORMAT}"
echo
echo "${MAGENTA_TEXT}${BOLD_TEXT}For more tutorials by Praveen Tech:${RESET_FORMAT}"
echo "${BLUE_TEXT}${UNDERLINE_TEXT}https://www.youtube.com/@PraveenTech1${RESET_FORMAT}"
